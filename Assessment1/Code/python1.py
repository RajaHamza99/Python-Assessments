...
def five(input):
	output = set()
	check = input.split(",")
	for x in range(0, int(len(check) / 4)):
		if check[(4 * x) + 2] == "False":
			output.add(check[(4 * x)])
	output = list(output)
	return output
...
def six(input):
	if ("ie" in input and not ("cie" in input)) or "cei" in input:
		return True
	return False
